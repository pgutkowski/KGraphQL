...
class HttpRequestHandler(val schema : Schema) : SimpleChannelInboundHandler<FullHttpRequest>() {
    ...
    override fun exceptionCaught(ctx: ChannelHandlerContext, cause: Throwable) {
        logger.log(Level.INFO, cause.message, cause)
        writeErrorMessage(cause, ctx)
    }

    private fun writeResponse(ctx: ChannelHandlerContext, response: String) {
    ...
    private fun writeErrorMessage(cause: Throwable, ctx: ChannelHandlerContext) {
        val httpResponse = DefaultFullHttpResponse(
                ...
                Unpooled.wrappedBuffer(objectMapper.writeValueAsBytes(cause))
        )
